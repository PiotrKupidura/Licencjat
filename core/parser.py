...
class Structure:
    ...
    def generate_observations(self, len_fragment):
        inputs = []
        labels = []
        for i in range(self.length() - len_fragment + 1):
            inp = np.zeros((len_fragment,3,3))
            lab = np.zeros((len_fragment,6))
            ignore = False
            ...
            if ignore:
                ignore = False
                continue
            dx = float(self._c[-1].x) - float(self._c[0].x)
            dy = float(self._c[-1].y) - float(self._c[0].y)
            dz = float(self._c[-1].z) - float(self._c[0].z)
            lab[:,-3:] = np.array([dx, dy, dz]*len_fragment).reshape(len_fragment,3)
